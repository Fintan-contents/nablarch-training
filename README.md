...
以下のコマンドを実行し、スキーマの作成、マスタデータ、テストデータの読み込み、及びエンティティクラスの生成を行ってください(なおこの作業は、各ハンズオン実施時に、それぞれのディレクトリに移動して実行する必要があります)。
...
続いて`mvn waitt:run`を実行し、http://localhost:9080 にアクセスしてください。ログイン後、正常に「プロジェクト検索画面」が表示されることを確認してください。
