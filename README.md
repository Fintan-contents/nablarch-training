...
### 1. リポジトリの作成
以下の6つのGitリポジトリを同ディレクトリ内に作成してください。<br />
ローカルで動作させるだけの場合は対象リポジトリの適切なブランチのZipをダウンロードし、ディレクトリに展開するだけでもかまいません。<br />

    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/doc.git
    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/tools.git
    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/nablarch-handson-app-parent.git
    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/nablarch-handson-app-core.git
    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/nablarch-handson-app-entity.git
    $git clone -b master http://nablarch.intra.tis.co.jp/gitbucket/git/nablarch-handson/nablarch-handson-app-web.git
  ...
  <settings>
    ...
    <proxies>
      <proxy>
        ...
        <host>tisproxy.intra.tis.co.jp</host>
        <port>8080</port>
        <!-- プロキシを経由しないホスト -->
...
### 4. nablarch-handson-app-core.jar, nablarch-handson-app-entity.jar の生成
以下のコマンドを順番に実行し、nablarch-handson-app-core.jar, nablarch-handson-app-entity.jar を作成してください。

```
    $cd <チェックアウトディレクトリ>
    $cd nablarch-handson-app-parent
    $mvn clean install
    $cd ../nablarch-handson-app-core
    $mvn clean install
    $cd ../nablarch-handson-app-entity
    $mvn clean install
```    


### 5. DBセットアップ
handson実行に必要なスキーマの作成および、マスタデータ、テストデータの読込を行います。
クローンした nablarch-handson-app-web ディレクトリに移動後、
`$mvn gsp-dba:import-schema`を実行して下さい。

DBは、組み込みDBのH2を使用しています。
データファイルは、homeディレクトリ(Windows 7の場合は、C:\Users\ <アカウント名> )に作成される以下のファイルです。

* nablarch_example.*.db

学習が終わって不要になった際には、削除してください。
...
| ユーザ名 | パスワード | 権限 |
|:------|:--------|:------|
| 90000000000000000001 | pass123- | 管理者 |
| 90000000000000000002 | pass123- | 管理者 |
| 90000000000000000003 | pass123- | 管理者 |
| 90000000000000000004 | pass123- | 管理者 |
| 90000000000000000005 | pass123- | 管理者 |
| 10000000000000000001 | pass123- | 一般 |
| 10000000000000000002 | pass123- | 一般 |
| 10000000000000000003 | pass123- | 一般 |
| 10000000000000000004 | pass123- | 一般 |
| 10000000000000000005 | pass123- | 一般 |
