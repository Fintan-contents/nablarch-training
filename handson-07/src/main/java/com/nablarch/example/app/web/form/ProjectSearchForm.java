...
public class ProjectSearchForm extends SearchFormBase implements Serializable {
    ...
    public String getSortId() {
        ...
        if (StringUtil.hasValue(sortKey) && StringUtil.hasValue(sortDir)) {
            // handson-07
            // SQL文で使用するsortIdを生成する処理を実装してください。生成したsortIdは、本メソッドの戻り値として使用します。
            // sortIdの書式は以下の通りです。
            // 今回sortIdは、ソートのキーにする項目を表す接頭辞と、昇順降順を表す接尾辞で構成します。
            //
            // 接頭辞については以下の仕様とします。
            // sortKey変数が"id"の時：
            //   "id"           (PROJECT_IDをキーにすることを示す)
            // sortKey変数が"name"の時：
            //   "name"         (PROJECT_NAMEをキーにすることを示す)
            // sortKey変数が"sdate"の時：
            //   "startDate"    (PROJECT_START_DATEをキーにすることを示す)
            // sortKey変数が"edate"の時：
            //   "endDate"      (PROJECT_END_DATEをキーにすることを示す)
            // sortKey変数が上記以外の時：
            //   "id"           (PROJECT_IDをキーにすることを示す)
            //
            // 接尾辞については以下の仕様とします。
            ...
            //   "Asc"
            //
            // 例えば、sortKey変数が"id"かつsortDir変数が"asc"の時に、sortIdは、"idAsc"となります。
        }
        return sortId;
    }
